...
lives in a new `RedactingMemoryStore` decorator the container wraps around your
memory driver; if you resolve `MemoryStore` you now receive the decorator (call
`->inner()` for the wrapped driver). Custom `MemoryStore` drivers need no changes
— they are wrapped transparently.
...
(preserving array shape and keys, the same convention the audit path uses);
`Skip` drops the entry entirely (no row, no `MemoryWritten` event). Because
redaction happens at the store, the propagation view and frozen `MemorySnapshot`
honor it automatically. See [docs/memory.md](docs/memory.md#capture-policy-write-time-redaction).
