...
Audit-path exception **messages** are redacted to `[redacted]` by default since v0.13.1 (`SWARM_AUDIT_REDACT_EXCEPTION_MESSAGES=true`) unless capture already permits failure free-text; the exception **class/type is always logged**, so failures stay diagnosable.
