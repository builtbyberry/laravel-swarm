...
| `RunContext` | Explicit run input, run ID, data, metadata, artifacts, labels, and details. `withAgentInput()` binds native message content and explicit recipient policy while retaining the textual input projection. Implements `ArrayAccess` — `$context['key']` reads and writes directly through to the `SwarmMemory` Run scope (v0.9.0+). `withConversationId()` binds the run to a conversation (read back with `conversationId()`), making `MemoryScope::Conversation` addressable for snapshot gathering and the `Recall`/`Remember` tools; the id threads through every execution path via run metadata (v0.12.0+). | [Native Messages and Attachments](native-inputs.md), [Structured Input](structured-input.md), [Persistence And History](persistence-and-history.md), [Swarm Memory](memory.md#runcontext-write-through), [Conversation-scoped memory](memory.md#conversation-scoped-memory) |
| `NativeInputRecipient` | Declares a topology-stable recipient, attachment selection, text source, and optional provider/model/timeout overrides. | [Native Messages and Attachments](native-inputs.md#explicit-recipients) |
