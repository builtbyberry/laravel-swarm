...
adding a new public API. The [v0.26.0 adoption index](ai-0112-release-evidence.md)
records preserved contracts, exact removal scope and unsupported new integrations.
...
| `ProviderToolData` | Bounded structured data with explicit availability; empty, withheld, partial and unavailable evidence remain distinct. | [Capture and bounds](provider-tool-events.md#capture-and-bounds) |
| `SwarmProviderToolAttemptInvalidated` | Durable replay control marker invalidating earlier provider observations for one node; it is not provider execution or approval continuation. | [Durable attempts](provider-tool-events.md#durable-attempts-and-transaction-boundaries) |
...
| `RunContext` | Explicit run input, run ID, data, metadata, artifacts, labels, and details. `withAgentInput()` binds native message content and explicit recipient policy; `withAgentConfiguration()` binds reconstructible native per-run settings to stable recipients. Implements `ArrayAccess` — `$context['key']` reads and writes directly through to the `SwarmMemory` Run scope (v0.9.0+). `withConversationId()` binds the run to a Swarm memory conversation, which is separate from Laravel AI native conversation continuation. | [Native Messages and Attachments](native-inputs.md), [Structured Input](structured-input.md), [Persistence And History](persistence-and-history.md), [Swarm Memory](memory.md#runcontext-write-through) |
| `NativeInputRecipient` | Declares a topology-stable recipient, attachment/text selection, provider/model/timeout, reconstructible tools, one-shot native messages, or one native conversation. | [Native Messages and Attachments](native-inputs.md#per-run-native-agent-settings) |
| `NativeAgentToolReference` | Reconstructible class plus plain constructor arguments used to rebuild a Laravel AI agent/tool/provider tool inside a worker. | [Native Messages and Attachments](native-inputs.md#reconstructible-tools) |
| `NativeAgentToolFactoryReference` | Stable configured factory identifier plus plain arguments. Expanded once at admission into `NativeAgentToolReference` values. | [Native Messages and Attachments](native-inputs.md#reconstructible-tools) |
| `NativeAgentConversation` | Request-local start or existing-conversation continuation descriptor. Recoverable execution requires an existing ID and Eloquent participant. | [Native Messages and Attachments](native-inputs.md#native-conversations) |
| `AuthorizesNativeInputAttachment` | Application authorization boundary for stored and provider-file references; the shipped binding denies external references by default. | [Native Messages and Attachments](native-inputs.md#operational-storage-and-ownership) |
| `AuthorizesNativeAgentConversation` | Application authorization boundary checked at admission and again before each native conversation invocation; the shipped binding denies all access. | [Native Messages and Attachments](native-inputs.md#native-conversations) |
| `NativeAgentToolFactory` | Application extension point that converts one registered factory call into frozen reconstructible tool references during admission. | [Native Messages and Attachments](native-inputs.md#reconstructible-tools) |
| `ConsumesNativeInputMessages` | Required companion contract for a custom `NativeInputStore` that supports recoverable one-shot messages; its transaction callback must atomically enclose terminal history and consumption on the same database connection. | [Native Messages and Attachments](native-inputs.md#operational-storage-and-ownership) |
...
| `#[ContextGrowthPolicy]` | Declare a streaming swarm's context-growth intent: `#[ContextGrowthPolicy(GrowthPolicy::…)]`. Overrides the global `swarm.context_growth.policy` for the swarm. Streaming substrate only — non-streaming `prompt()` runs are out of scope. (v0.15.0+) | [Streaming Substrate Author Guide](streaming-substrate-author-guide.md#context-growth-policy) |
| `#[DurableStreaming]` | Opt a swarm into durable per-node streaming: a durable run streams each node's events into the causal log instead of one blocking `prompt()` per node. The decision is resolved once and pinned onto the run row at run-start, so a run streams (or not) for its whole life regardless of a redeploy. A bare `#[DurableStreaming]` opts in; `#[DurableStreaming(false)]` opts out. Absent ⇒ off. Streams on every durable topology — sequential, hierarchical, static_hierarchical, and parallel (including fan-out branches); the hierarchical coordinator streams structural events (token streaming is a follow-up). Opting in on a future, not-yet-wired topology fails loud at dispatch. (v0.15.0+) | [Durable Execution](durable-execution.md#durable-per-node-streaming) |
...
| `BuiltByBerry\LaravelSwarm\Testing\ScriptedAgent` | Abstract deterministic offline helper. Subclasses implement `instructions(): string` and `reply(string $prompt): string`; `prompt()` returns a native response-shaped value. Used by starter examples and retained compatibility scaffolds. It is not the normal model-agent path or test-double surface; use Laravel AI `make:agent` and the generated class's `fake()` method for those. (v0.8.0+) | [Examples](examples.md), [Generators](generators.md), [Testing](testing.md) |
...
| `swarm:install:examples` | Copy curated starter examples from `stubs/examples/` into `app/Ai/Swarms/<Name>/` **verbatim** (fixed-name reference copies), rewriting namespace placeholders. Flags: `--example=<name>` (repeatable), `--all`, `--force`. (v0.8.0+) | [Examples](examples.md) |
| `make:swarm:blueprint <Name>` | Scaffold a **complete, runnable** swarm from a curated blueprint — the swarm class, its agents, and a console command — from the same `stubs/examples/` corpus `swarm:install:examples` uses, but **renamed** as your own (swarm class, namespace segment, and console command renamed; agent class names preserved). Catalog: `pipeline`, `research`, `triage`, `extraction`, `approval`, `memory`, `streaming`. Flags: `--template=<slug>` (required non-interactively; interactive picker otherwise), `--without-command`, `--force`. Fails loud on collision (checks the whole plan before writing), reserved/uncompilable names, and missing `--template`. (v0.18.0+) | [Generators](generators.md#make-swarm-blueprint) |
| `make:agent` + `make:swarm:swarm` | Normal authoring pair. Laravel AI generates native model agents, including `--structured`; Swarm generates the orchestration class. `make:swarm:agent` remains a stable deterministic-offline compatibility command and continues honoring `stubs/swarm.agent.stub`. (v0.28.0+) | [Generators](generators.md), [Native Agent Onboarding](native-agent-onboarding.md) |
...
| `swarm:health` | Verify configured stores, durable tables, audit-outbox state, and runtime readiness. Options: `--durable` (include durable infrastructure), `--audit` (audit-outbox checks only; v0.5.0+), `--parallel-streaming` (exercise the provider-free process-child and authenticated loopback handshake; v0.28.0+). `--audit` and `--parallel-streaming` are mutually exclusive. | [Getting Started](getting-started.md), [Maintenance](maintenance.md), [Parallel Live Multiplexing](streaming.md#parallel-live-multiplexing) |
| `swarm:upgrade` | Preview the v0.25-to-v0.26 dependency recipe; explicitly apply selected manifest fixes with `--apply`, `--expect` and `--yes`, or guard a manifest restore with `--restore`. Includes `--path`, `--json` and a standalone `swarm-upgrade` entry point. | [Upgrade assistant](upgrade-assistant.md) |
...
| `swarm:relay` | Drain the durable outbox and audit outbox in a single pass under the configured finite command-overlap lease, dispatching queued step/branch jobs and replaying failed audit evidence. Must be scheduled (`everyMinute()`). Options: `--type=step\|branch\|queued_resume\|audit` (v0.5.0 adds the `audit` lane), `--limit=N`, `--drain-until-empty`, `--max-attempts=N`. | [Maintenance](maintenance.md#command-overlap-leases) |
| `swarm:recover` | Redispatch recoverable durable work under the configured finite command-overlap lease. | [Durable Execution](durable-execution.md#pause-resume-cancel-and-recover), [Command overlap leases](maintenance.md#command-overlap-leases) |
| `swarm:prune` | Remove expired database persistence rows. | [Maintenance](maintenance.md#pruning-expired-records) |
| `swarm:compact` | Discover streaming runs with a sealed window and dispatch a `CompactSwarmRun` queue job per run to graduate the sealed hot-event prefix to cold storage. **Must be explicitly scheduled** by the consumer app (`->hourly()` recommended); the package does not auto-schedule it. Database driver only. Flags: `--run-id=<id>` (target one run), `--limit=50` (max runs discovered per sweep). (v0.15.0+) | [Operator Runbook: Streaming Substrate](operator-runbook-streaming-substrate.md), [Maintenance](maintenance.md#scheduling) |
...
| `IdentifiesSigningKey` | Opt-in companion to `SwarmAuditSigner` that adds `keyId(): ?string`. When a signer implements it and returns a non-empty, non-secret id, the dispatcher stamps the package-standardized `signature_key_id` field onto signed records so sinks can select the right verification key across a rotation window. Dispatcher-owned (signers MUST NOT set the field in `sign()`); a throwing `keyId()` is treated as absent. (v0.16.0+) | [Audit Evidence Contract](audit-evidence-contract.md#exposing-the-key-id) |
...
| `StreamStepCheckpointStore` | Contract recording completed non-final sequential step output + usage. A successfully persisted, readable checkpoint skips that step on resume; best-effort write failures and final/unfinished work permit re-execution. Methods: `record(runId, stepIndex, output, usage)` — upsert the completed checkpoint; `find(runId, stepIndex)` — return the completed `StreamStepCheckpoint` or `null` (a step that crashed before completion reads as absent). Bound in lockstep with `SnapshotsMemory` (database driver → `DatabaseStreamStepCheckpointStore`; cache driver → no-op `NullStreamStepCheckpointStore`). The non-durable analogue of the durable runtime's per-node output store. Used by `SequentialRunner` on the streamed path. (v0.12.0+) | [Streaming — Crash-Replay Durability](streaming.md#crash-replay-durability) |
...
| `Concerns\HasSwarmFilesystemTools` | Opt-in trait exposing `swarmFilesystemTools(): array` for an agent's `tools()`. Returns Laravel AI's filesystem tools (`ReadFile`, `WriteFile`, `ListFiles`, `DeleteFile`, `CopyFile`, `FileExists`, `GetFileMetadata`, `GetFileUrl`), each bound to `swarm.filesystem.tools.disk`, only when `enabled` is true **and** a `disk` is set (no default — inert without one); honours the per-tool toggles. Operations are disk-relative and confined to the disk root. (v0.21.0+) | [Filesystem Agent Tools](filesystem-tools.md) |
...
| `Streaming\View\VoidedEvent` | Wrapper emitted in the `ViewSupersession::Everything` fold for an event a void-edge voided. Properties: `event` (`SwarmStreamEvent`), `voidType` (`CausalVoidEdgeType`), `reason` (string — taken from the most recent void-edge in causal order), `digestNodeId` (`?string` — the rollup summary node for a `RolledUp` edge; `null` for every other void type). (v0.15.0+) | [Lifecycle Events](events.md#causal-void-edges-282-289) |
| `Streaming\Events\CausalVoidEdgeType` | Enum of void-edge kinds the fold interprets. Cases: `Supersedes` (`supersedes`), `Replaces` (`replaces`), `Abandons` (`abandons` — suppresses the target and its node subtree), `RolledUp` (`rolled_up` — suppresses only the digested node's own events, #289). (v0.15.0+) | [Lifecycle Events](events.md#causal-void-edges-282-289) |

## Streaming Substrate (v0.15.0+)
...
> public in v0.17.1 (#349), stable for two minors since their v0.15.0
...
| `swarm.compaction.lease_seconds` | `300` | `SWARM_COMPACTION_LEASE_SECONDS` | How long a per-run compaction lease is held. The lease expires naturally if the job process dies. | v0.15.0+ |
| `swarm.context_growth.policy` | `degrade_to_cold` | `SWARM_CONTEXT_GROWTH_POLICY` | Framework default growth-policy rung when a swarm declares no `#[ContextGrowthPolicy]`. | v0.15.0+ |
| `swarm.context_growth.budget_events` | `null` (inert) | `SWARM_CONTEXT_GROWTH_BUDGET_EVENTS` | Soft budget in hot stream events. `null` leaves the policy inert; supplying a number activates the default `degrade_to_cold` behaviour unless an author declares another rung. | v0.15.0+ |
| `swarm.context_growth.hard_cap_events` | `null` (disabled) | `SWARM_CONTEXT_GROWTH_HARD_CAP_EVENTS` | Absolute ceiling that clamps author intent — a breach refuses the run regardless of declared policy (even `ignore`). Best-effort governance, not a correctness invariant. | v0.15.0+ |
| `swarm.context_growth.backpressure_delay_ms` | `250` | `SWARM_CONTEXT_GROWTH_BACKPRESSURE_DELAY_MS` | Bounded delay inserted at a step boundary under the `backpressure` rung. Clamped to a safe maximum. | v0.15.0+ |
| `swarm.hierarchical.stream_parallel_branches` | `concurrent` | `SWARM_HIERARCHICAL_STREAM_PARALLEL_BRANCHES` | Concurrent vs. sequential parallel-branch streaming for streamed `Hierarchical` swarms (`#[StreamParallelBranches]` overrides per swarm). | v0.15.0+ |
| `swarm.streaming.parallel.enabled` | `false` | `SWARM_PARALLEL_STREAMING_ENABLED` | Opt into process-backed top-level parallel live multiplexing. No buffered fallback. | v0.28.0+ |
| `swarm.streaming.parallel.max_branches` | `32` | `SWARM_PARALLEL_STREAMING_MAX_BRANCHES` | Per-stream branch-process admission bound (1–256); not a raw FD or application-wide ceiling. | v0.28.0+ |
| `swarm.streaming.parallel.max_frame_bytes` | `2097152` | `SWARM_PARALLEL_STREAMING_MAX_FRAME_BYTES` | Atomic branch frame byte bound (1 KiB–4 MiB). | v0.28.0+ |
| `swarm.streaming.parallel.cancel_grace_milliseconds` | `250` | `SWARM_PARALLEL_STREAMING_CANCEL_GRACE_MILLISECONDS` | Grace before sibling processes are forcibly stopped during cleanup (0–10000 ms). | v0.28.0+ |
| `swarm.durable.streaming_enabled` | `true` | `SWARM_DURABLE_STREAMING_ENABLED` | Operator kill-switch for durable per-node streaming. Per-swarm opt-in is `#[DurableStreaming]`; this gates only emission, so when off opted-in runs fall back to `prompt()` while crashed attempts are still voided and committed nodes still sealed. Safe to flip mid-run. | v0.15.0+ |
