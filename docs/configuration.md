...
Controls the command-owned finite leases used by `swarm:relay` and
`swarm:recover`. Scheduler examples derive their whole-minute mutex expiry from
the same lease duration.
...
| `swarm.commands.overlap.lease_seconds` | int | `3600` | `SWARM_COMMAND_OVERLAP_LEASE_SECONDS` | Finite command lease. Must be positive and longer than the worst-case invocation. Direct invocation becomes startable at exact expiry; recommended scheduler mutexes round the duration up to whole minutes. A command that outlives the lease is no longer protected. |
...
| `swarm.native_agent_settings.enabled` | bool | `false` | `SWARM_NATIVE_AGENT_SETTINGS_ENABLED` | Admit new v2 per-run tool, message-history and conversation settings. Existing v2 references remain readable while disabled. Enable only after every worker has v2 readers. |
...
| `swarm.audit.redact_exception_messages` | bool | `true` | `SWARM_AUDIT_REDACT_EXCEPTION_MESSAGES` | Redact exception **messages** embedded in audit sink-failure / signing / outbox log paths. Provider/driver/tool exception messages can carry prompt fragments or PII. When `true` (default), the message logs as `[redacted]` unless capture already permits failure free-text (`swarm.capture.inputs` **and** `swarm.capture.outputs` both enabled). Set `false` to always log the raw message regardless of capture. The exception **class/type is always logged** either way. See [Redaction and Capture Alignment](audit-evidence-contract.md#redaction-and-capture-alignment). |
