...
Customize Swarm-owned generator output by publishing its stubs into your project:

```bash
php artisan vendor:publish --tag=swarm-stubs
```

This includes `swarm.agent.stub` and `swarm.single-agent.stub`; existing
published copies remain authoritative for their compatibility commands and are
never rewritten during upgrade.

Native model agents are generated by Laravel AI. Customize that separate stub
family with:
...
That publishes Laravel AI's `agent.stub` and `structured-agent.stub` (alongside
its other generator stubs). The Swarm and Laravel AI generators each consult
only their own published filenames. See [Generators](./generators.md) for the
full surface.
