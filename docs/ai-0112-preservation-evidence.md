...
The component PR must attach exact-head CI results for both real-database jobs
and the existing PHP/dependency/coverage matrix before merge. These CI results
are separate from the local runs above; no local MySQL/Postgres execution is
claimed. The PR check record is the landing evidence for D and CI coverage.
...
| A06 | [SwarmFake](../tests/Unit/SwarmFakeTest.php) covers inert intent/proxies; [queued coordination](../tests/Feature/QueuedHierarchicalParallelCoordinationTest.php) and [C2 job boundaries](../tests/Feature/NativeOutcomeBoundaryTest.php) execute real jobs. | T, P |
| A07 | [Native result parity](../tests/Feature/Streaming/NativeResultParityTest.php), [native HTTP](../tests/Feature/Streaming/NativeHttpParityTest.php), [historical reader](../tests/Unit/Streaming/NativeResultReaderTest.php) and telemetry/causal lanes retain C3 identities, correction flags, capture and bounded interleaving. | T, P, D |
...
28 restored negative controls: 27 assertion failures and one runtime failure
...
|---|---|---|---|
| max-tokens-wire | `vendor/laravel/ai/src/Gateway/OpenAi/Concerns/BuildsTextRequests.php` | `successful usage` | Assertion failure |
| temperature-wire | `vendor/laravel/ai/src/Gateway/OpenAi/Concerns/BuildsTextRequests.php` | `successful usage` | Assertion failure |
| top-p-wire | `vendor/laravel/ai/src/Gateway/OpenAi/Concerns/BuildsTextRequests.php` | `successful usage` | Assertion failure |
| provider-options-wire | `vendor/laravel/ai/src/Gateway/OpenAi/Concerns/BuildsTextRequests.php` | `successful usage` | Assertion failure |
| structured-schema-wire | `vendor/laravel/ai/src/Gateway/OpenAi/Concerns/BuildsTextRequests.php` | `structured JSON` | Assertion failure |
| tool-result-pair-wire | `vendor/laravel/ai/src/Gateway/OpenAi/Concerns/BuildsTextRequests.php` | `paired tool` | Assertion failure |
| discovery-wire | `vendor/laravel/ai/src/Gateway/OpenAi/Concerns/MapsTools.php` | `paired tool` | Assertion failure |
| deferred-wire | `vendor/laravel/ai/src/Gateway/OpenAi/Concerns/MapsTools.php` | `paired tool` | Assertion failure |
| stateless-restriction | `vendor/laravel/ai/src/Gateway/OpenAi/Concerns/MapsTools.php` | `discovery restrictions` | Assertion failure |
| unsupported-discovery | `vendor/laravel/ai/src/Gateway/TextGenerationLoop.php` | `discovery restrictions` | Assertion failure |
| duplicate-discovery | `vendor/laravel/ai/src/Gateway/TextGenerationLoop.php` | `discovery restrictions` | Assertion failure |
| middleware-order | `vendor/laravel/ai/src/Providers/Concerns/GeneratesText.php` | `successful usage` | Assertion failure |
| middleware-bypass | `vendor/laravel/ai/src/Providers/Concerns/GeneratesText.php` | `failure ordering` | Assertion failure |
| guardrail-input | `src/Runners/SwarmGuardrailRunner.php` | `failure ordering` | Assertion failure |
| guardrail-step | `src/Runners/SwarmGuardrailRunner.php` | `failure ordering` | Assertion failure |
| guardrail-output | `src/Runners/SwarmGuardrailRunner.php` | `failure ordering` | Assertion failure |
| conversation-opt-in | `vendor/laravel/ai/src/Middleware/RememberConversation.php` | `conversation opt in` | Assertion failure |
| conversation-role-wire | `vendor/laravel/ai/src/Gateway/OpenAi/Concerns/MapsMessages.php` | `conversation opt in` | Assertion failure |
| structured-input-wire | `tests/Feature/Adoption/Fixtures/NativeWire.php` | `structured JSON` | Assertion failure |
| concrete-store-bound | `src/Runners/DispatchValidator.php` | `concrete durable` | Assertion failure |
| signal-duplicate-bound | `src/Runners/Durable/DurableSignalHandler.php` | `record before release` | Assertion failure |
| signal-capture | `src/Runners/Durable/DurablePayloadCapture.php` | `signal plaintext` | Assertion failure |
| signal-wire | `src/Persistence/DatabaseDurableRunStore.php` | `signal plaintext` | Assertion failure |
| child-claim-bound | `src/Persistence/DatabaseDurableRunStore.php` | `age reclaim` | Assertion failure |
| workflow-retry | `src/Runners/Durable/DurableRetryHandler.php` | `repeat a native tool` | Assertion failure |
| route-worker-allowlist | `src/Routing/HierarchicalRoutePlanner.php` | `structured JSON` | Assertion failure |
| route-cycle | `src/Routing/HierarchicalRoutePlanner.php` | `structured JSON` | Runtime recursion failure |
| signal-record-loss | `src/Runners/Durable/DurableSignalHandler.php` | `record before release` | Assertion failure |
