...
- `queue()` defaults to one Laravel queue job owning the workflow. Generated hierarchical swarms can opt into database-backed `multi_worker` branch/join coordination; static hierarchy retains its in-process queued path. See [queued hierarchy](docs/hierarchical-routing.md#queue). Queued swarms are re-resolved from the container, so they must not rely on runtime instance state. Pass per-run data in the task payload or `RunContext`.
- `stream()` supports sequential, generated hierarchical (synchronous coordinator, streamed workers), and static hierarchical swarms. Top-level parallel live multiplexing is default-off (`swarm.streaming.parallel.enabled`) and requires Laravel's `process` concurrency driver; its branch events carry explicit branch/attempt/sequence identity and have no invented global order. See [SwarmRunner](src/Runners/SwarmRunner.php) and [stream validation](src/Runners/DispatchValidator.php). It returns a lazy `StreamableSwarmResponse`, emits typed progress and final streamed output, and supports in-memory replay after completion. Persisted replay is opt-in via `storeForReplay()` or `swarm.streaming.replay.enabled`; replay with `SwarmHistory::replay($runId)`. Replay write failures default to failing the stream, unless `swarm.streaming.replay.failure_policy` is set to `continue`; in continue mode, already-written replay events are discarded so partial playback is unavailable. For upstream final-agent streamed events, persisted replay preserves upstream IDs and timestamps; invocation IDs are passed through when present. `broadcast()`, `broadcastNow()`, and `broadcastOnQueue()` are stream-event helpers for those same supported streaming topologies; they broadcast typed swarm stream events, not lifecycle events for every topology.
- `dispatchDurable()` is database-backed, checkpointed execution for sequential, parallel, generated hierarchical, and static hierarchical swarms. It persists a durable cursor and advances the swarm through durable jobs. Use events such as `SwarmCompleted` and `SwarmFailed`; durable responses do not support `then()` or `catch()`.

Queued whole-workflow `then()` and `catch()` callbacks are unavailable: [QueuedSwarmResponse](src/Responses/QueuedSwarmResponse.php) only proxies methods declared on its pending dispatch. Use `SwarmCompleted` and `SwarmFailed`. Stream `each()` / `then()` remain supported by [StreamableSwarmResponse](src/Responses/StreamableSwarmResponse.php).
...
- **Hierarchical:** the first agent is the coordinator. It must use Laravel AI structured output and return the full route plan. Laravel Swarm validates the plan as a DAG and executes `worker`, `parallel`, and `finish` nodes. In `prompt()`, parallel groups execute concurrently. In default `queue()` mode, hierarchical parallel groups execute sequentially in declaration order while retaining parallel-safe validation rules. Opt-in `multi_worker` uses branch jobs and a coordinated join. [ResumeQueuedHierarchicalSwarm](src/Jobs/ResumeQueuedHierarchicalSwarm.php) uses [ConfiguresDurableAdvanceJob](src/Jobs/Concerns/ConfiguresDurableAdvanceJob.php), not the ordinary queued-job retry profile.
...
- **Capture defaults** — Shipped `config/swarm.php` defaults `swarm.capture.inputs`, `outputs`, `artifacts`, and `active_context` to **false** so prompts and agent payloads are not persisted unless the application opts in (`SWARM_CAPTURE_*` or config). Treat prompts, outputs, lifecycle events, streamed reasoning/tool payloads, and automatic step artifacts as sensitive whenever capture is enabled. With output capture off, streamed function-tool/reasoning payloads redact values to `[redacted]` while preserving keys where applicable. Under Redact, provider-tool payloads withhold the entire arbitrary structure, including keys; see [provider-tool capture and bounds](docs/provider-tool-events.md#capture-and-bounds).
...
- Timeouts are best-effort orchestration deadlines. Ordinary modes check before and between steps. Process-backed top-level parallel streaming also enforces the absolute deadline while multiplexing and terminates local branch processes; neither path guarantees cancellation of remote provider work already accepted.
