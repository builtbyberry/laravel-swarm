...
- **In-process vs. background.** The class-free builders (`Swarm::agent()`/`sequential()`/`parallel()`/`hierarchical()`) run **in-process** — `prompt()`/`run()`/`stream()`/`broadcast()`. For queued or durable execution, author a `Swarm` class (`php artisan make:swarm:swarm YourSwarm`, returning a single agent from `agents()` — there is no one-agent flag); background runs need a container-bound class to survive job re-resolution. `stream()`/`broadcast()` need a streamable topology (sequential/hierarchical/static-hierarchical, not parallel).
...
- **Agents are `laravel/ai` agents.** Any class implementing `Laravel\Ai\Contracts\Agent` works, unchanged — Swarm type-hints the vendor contract everywhere. Use `php artisan make:swarm:agent`. (`BuiltByBerry\LaravelSwarm\Contracts\Agent` is a deprecated alias kept for pre-v0.23.0 code; do not write new agents against it.)
