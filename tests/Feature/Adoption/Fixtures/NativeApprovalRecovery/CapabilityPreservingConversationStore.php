...
class CapabilityPreservingConversationStore implements ConversationStore, ResolvesPendingApprovals, VerifiesConversationOwnership
{
    /** @var list<array<int, array<string, mixed>>> */
    public array $approvalWrites = [];

    public function __construct(private readonly ConversationStore&ResolvesPendingApprovals&VerifiesConversationOwnership $inner) {}
    ...
    {
        return $this->inner->conversationBelongsTo($conversationId, $participantType, $participantId);
    }
    ...
    {
        $this->approvalWrites[] = array_map(fn (ToolResult $result): array => $result->toArray(), $toolResults);
    ...
    {
        return $this->inner->pendingApprovalsFor($conversationId);
    }
}
