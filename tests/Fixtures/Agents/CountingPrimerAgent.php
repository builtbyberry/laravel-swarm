...
/**
 * A non-final step whose output DEPENDS on how many times it has been invoked:
 * the Nth invocation returns `primed-N`. This lets the #202 multi-step resume
 * tests prove that a skipped step's output is REHYDRATED from its checkpoint
 * (so a resumed run still feeds `primed-1` downstream) rather than RECOMPUTED
 * (which a re-execution would turn into `primed-2`).
 *
 * `$sideEffects` models an external tool side effect (a DB write / API call):
 * it is bumped once per invocation so a test can assert the side effect fired
 ...
 */
class CountingPrimerAgent implements Agent
{
    /** Count of prompt() invocations across a test. Reset in beforeEach. */
    public static int $invocations = 0;

    /** Count of (simulated external) side effects fired. Reset in beforeEach. */
    ...
    {
        return 'You produce a counter-dependent value for the next agent.';
    }
    ...
    public function prompt(AgentInput|UserMessage|Decisions|string $prompt, array $attachments = [], Lab|array|string|null $provider = null, ?string $model = null, ?int $timeout = null): AgentResponse
    {
        ...
        return new AgentResponse(
            ...
            usage: new TextUsage(inputTokens: 7, outputTokens: 11),
