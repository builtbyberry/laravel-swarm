...
/**
 * Final streaming agent that echoes its INPUT prompt back as a single
 * `TextDelta`. Because its streamed output is exactly the prompt it received —
 * which in a sequential swarm is the previous step's `last_output` — it makes
 * the downstream prompt observable in the event stream. The #202 tests use it
 * to prove a resumed run feeds the byte-identical, rehydrated upstream output
 * (`primed-1`) and not a recomputed value (`primed-2`).
 *
 * Event ids/timestamp are fixture-fixed so the serialized text events are
 * byte-stable across runs once the per-run run_id is normalised out.
 *
 ...
 */
class EchoStreamingAgent implements Agent
{
    public function instructions(): Stringable|string
    {
        return 'You echo whatever the previous step produced.';
    }
    ...
    {
        return new StreamableAgentResponse('echo-streaming-invocation', function () use ($prompt): \Generator {
            $timestamp = 1_710_000_000;

            yield new TextDelta('echo-delta', 'echo-message', $prompt, $timestamp);
            yield new TextEnd('echo-text-end', 'echo-message', $timestamp);
            yield new StreamEnd('echo-stream-end', 'stop', new Usage(promptTokens: 1, completionTokens: 1), $timestamp);
        }, new Meta('fake', 'test'));
    }
