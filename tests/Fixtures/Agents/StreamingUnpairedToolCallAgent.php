...
/**
 * Final streaming agent that emits a `ToolCall` event with **no** matching
 * `ToolResult` before the stream ends — modelling a partial/interrupted tool
 * invocation. The runner holds the call in its pending set and, at stream end,
 * flushes it into the step snapshot with a null result so replay can detect the
 * partial run. No real tool is invoked: the runner pairs purely from stream
 * events, so emitting the bare `ToolCall` is enough to exercise the unpaired
 * snapshot path.
 *
 ...
 */
class StreamingUnpairedToolCallAgent implements Agent
{
    public function instructions(): Stringable|string
    {
        return 'You call a tool but the stream ends before its result arrives.';
    }
    ...
    public function prompt(string $prompt, array $attachments = [], Lab|array|string|null $provider = null, ?string $model = null, ?int $timeout = null): AgentResponse
    {
        return new AgentResponse(
            invocationId: 'streaming-unpaired',
            text: 'unused',
            usage: new Usage,
    ...
    {
        return new StreamableAgentResponse('streaming-unpaired-invocation', function (): \Generator {
            ...
            $toolCall = new ToolCallData(
                ...
                name: 'remember',
                arguments: $arguments,
                resultId: 'unpaired-result-1',
            );

            // A ToolCall with no following ToolResult: the stream ends with the
            // call still pending.
            yield new ToolCall('tool-call-unpaired', $toolCall, $timestamp);
            yield new TextDelta('delta-unpaired', 'message-unpaired', 'thinking', $timestamp);
            yield new TextEnd('text-end-unpaired', 'message-unpaired', $timestamp);
            yield new StreamEnd('stream-end-unpaired', 'stop', new Usage(promptTokens: 1, completionTokens: 1), $timestamp);
        }, new Meta('fake', 'test'));
    }
