...
class InstallDurableCommand extends Command
{
    ...
    private function renderScheduleBlock(string $existing): string
    {
        ...
        if ($needsImport) {
            ...
            $lines[] = '';
            $lines[] = "SwarmDurableSchedule::command('swarm:relay')->everyMinute()->withoutOverlapping(60)->runInBackground();";
            $lines[] = "SwarmDurableSchedule::command('swarm:recover')->everyFiveMinutes()->withoutOverlapping(60)->runInBackground();";
            $lines[] = "SwarmDurableSchedule::command('swarm:prune')->daily()->runInBackground();";
        } else {
            $lines[] = "Schedule::command('swarm:relay')->everyMinute()->withoutOverlapping(60)->runInBackground();";
            $lines[] = "Schedule::command('swarm:recover')->everyFiveMinutes()->withoutOverlapping(60)->runInBackground();";
